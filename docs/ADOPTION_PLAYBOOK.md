...
> Cross-app adoption strategy for Ideas, Prose, Tutor. Legion made the technical hard calls; owner decisions are noted inline.
...
- Prod Data Protection hosting per app: confirm each app's prod target so ConfigureDataProtection can be filled (Ideas has no Azure wiring shown; Prose/Tutor are Azure App Service Windows -> ProtectKeysWithAzureKeyVault + PersistKeysToAzureBlobStorage). The library is fail-closed in prod â€” no DP config = no boot.

- Decommission well-known accounts: confirm OK to deactivate admin@prose.local (Prose) and force-reset all 'aaa' accounts (Tutor) and let AuthBootstrapper mint a clean admin. Verify no runbook/automation depends on those exact identities.
...
- MFA scope beyond Admin: should Prose Contributors and/or Tutor Students be forced to enroll MFA? Current plan: MFA forced for Admin only; others password-only.
...
- Where does Psst run in Azure App Service Windows prod â€” is the Psst email channel reachable/credentialed there, or should prod fall back to SMTP (Prose's existing EmailService) while Psst is used only on local Windows dev hosts?
...
## Unified Adoption Playbook â€” MindAttic.Authentication across Ideas, Prose, Tutor
...
### Phase 1 â€” Prose (FIRST)
- Add package; ensure CI restore finds C:\\LocalNuGet (add NuGet source or drop .nupkg into ./lib/local-packages).
- ProseDbContext : IAuthDataContext (8 DbSets + ApplyMindAtticAuthConfiguration at top of OnModelCreating).
...
- Data migration MoveUsersToAuthUsers from `users.accounts` JSON blob: UserName=Email, NormalizedUserName/Email/NormalizedEmail set, EmailVerified=true, Role Administrator->Admin, LegacyHashScheme=bcrypt, new SecurityStamp; Admin rows MustEnrollMfa=true. DECOMMISSION admin@prose.local: deactivate it and let AuthBootstrapper mint a clean admin from bootstrap-token. Run from ApplyMigrations (rides the OIDC db_ddladmin job); GRANT INSERT/UPDATE/SELECT on schema auth to the runtime managed identity.
